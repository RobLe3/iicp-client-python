...
- Published the coordinated Python SDK line alongside TypeScript and Rust.
- No Python runtime behavior or wire semantics changed from 0.7.106.
...
### Changed — release recovery
- Hardened the manual release fallback and pinned recovery guidance to the current immutable release lane.
...
- `iicp-node service install`, `status`, `restart`, and `uninstall` now execute the selected launchd or systemd user-service lifecycle unless `--dry-run` is explicit.
