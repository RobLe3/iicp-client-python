#!/usr/bin/env python3
"""Run one exact, content-free pre-1.0 qualification case for client-python."""
...
RUNTIMES = ['cpython-3.11', 'cpython-3.12', 'cpython-3.13', 'cpython-3.14']
...
def _case_command(value: object, label: str) -> list[str]:
    ...
    exact_pytest = (
        ...
        and command[4].startswith("tests/")
        and command[4].split("::", 1)[0].endswith(".py")
        and command[4].endswith(f"::{assertion}")
    )
    if (
        ...
        raise RuntimeError(f"qualification case is not an exact pytest assertion: {label}")
    return command
