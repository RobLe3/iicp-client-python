...
class Pre1CandidateArtifactBuilderTest(unittest.TestCase):
    def test_description_is_content_free_and_complete(self) -> None:
        value = json.loads(subprocess.check_output([sys.executable, str(SCRIPT), "--describe"], text=True))
        self.assertEqual(value["component"], "client-python")
        self.assertEqual(value["artifact_identities"], [["wheel", "any"], ["sdist", "any"]])
