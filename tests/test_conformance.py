...
class TestOrchestrator:
    ...
    async def test_runs_all_four_concurrently_and_counts_results(self):
        ...
        assert report.pass_count == 4, f"failures: {[(r.test_id, r.message) for r in report.tests if not r.passed]}"
        assert report.fail_count == 0
        assert {r.test_id for r in report.tests} == {
            "CONF-REG-01", "CONF-HEALTH-01", "CONF-REACH-01", "CONF-DISC-01"
        }
